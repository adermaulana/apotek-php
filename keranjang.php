...
if (isset($_GET['hal']) && $_GET['hal'] == "hapus") {
    ...
    // Retrieve the quantity and id_obat from data_keranjang
    ...
    $id_obat = $data_keranjang['id_obat'];

    // Delete the item from data_keranjang
    $hapus = mysqli_query($koneksi, "DELETE FROM data_keranjang WHERE id_keranjang = $id_keranjang");

    if ($hapus) {
        // Update stok_obat in data_obat
        $update_stok = mysqli_query($koneksi, "UPDATE data_obat SET stok_obat = stok_obat + $jumlah_keranjang WHERE id_obat = $id_obat");
        ...
        if ($update_stok) {
            ...
            echo "<script>
                alert('Gagal mengupdate stok obat!');
                document.location='keranjang.php';
            </script>";
        }
    } else {
...
<div class="container mt-5">
    ...
    ?>
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                <th scope="row"><?= $no++ ?></th>
                                <td>
                                    <img src="<?= $data['gambar_obat']; ?>" alt="<?= $data['nama_obat']; ?>" class="img-thumbnail" style="max-width: 100px;">
                                    <?= $data['nama_obat']; ?>
                                </td>
        ...
        <a href="obat.php" class="btn btn-success">Lanjut Belanja</a>
    <?php } else { ?>
        <p>Tidak ada produk dalam keranjang belanja.</p>
        <a href="obat.php" class="btn btn-success">Mulai Belanja</a>
    <?php } ?>
</div>
